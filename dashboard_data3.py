...
with open(r"C:\Users\user\Downloads\data\data\DATA3\books.yaml", "r") as f:
    data = yaml.safe_load(f)

books_df = pd.DataFrame(data)
books_df.columns = [col.lstrip(":") for col in books_df.columns]

orders_df = pd.read_parquet(r"C:\Users\user\Downloads\data\data\DATA3\orders.parquet")
...
with tab2:
    ...
    st.subheader("Best buyer (with aliases)")
    st.write(list(top_customer.index))

with tab3:
    st.subheader("Number of unique sets of authors")
    st.write(unique_author_sets)

    st.subheader("Most popular author(s)")
    st.write(popular_authors.index.tolist())
